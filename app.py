...
class AppApicola:
    def __init__(self, root):
        ...
        self.ui_font = ("Arial Black", 12)
        self.style = ttk.Style()
        self.style.configure(".", font=self.ui_font)
        self.style.configure("Treeview.Heading", font=self.ui_font)
        self.style.configure("Treeview", font=self.ui_font)
    ...
    def setup_entidad(self, tab, nombre_entidad, tabla_db, cmd_guardar, cmd_modificar, cmd_eliminar, cmd_limpiar):
        ...
        ent_busq = ttk.Entry(f_busq, font=self.ui_font)
        ...
        for i, col in enumerate(cols[1:]):
            ...
            ent_dict[col] = ttk.Entry(f_form, font=self.ui_font)
    ...
    def modificar_cliente(self):
        if self.id_clie_edit is None: return
        d = [self.ent_clie[c].get().strip() for c in ["Nombre", "Apellido", "CUIT/DNI", "Teléfono", "Provincia", "Localidad"]]
        cuit_dni = d[2]
    ...
    def modificar_proveedor(self):
        if self.id_prov_edit is None: return
        d = [self.ent_prov[c].get().strip() for c in ["Nombre", "Apellido", "CUIT/DNI", "Teléfono", "Provincia", "Localidad"]]
        cuit_dni = d[2]
    ...
    def setup_stock(self):
        ...
        ttk.Label(self.stk_scroll_f, text="Material", width=40).grid(row=0, column=0, padx=10, pady=5)
        ttk.Label(self.stk_scroll_f, text="Cantidad", width=15).grid(row=0, column=1, padx=10, pady=5)
        ...
        for i, mat in enumerate(self.lista_materiales):
            ...
            ent = ttk.Entry(self.stk_scroll_f, width=15, justify="center", font=self.ui_font)
    ...
    def setup_ventas(self):
        ...
        self.v_ent_busq = ttk.Entry(f_clie, font=self.ui_font)
        self.v_ent_busq.pack(fill="x", padx=5, pady=5)
        self.v_ent_busq.bind("<KeyRelease>", lambda e: self.actualizar_tablas())

        self.v_list_clie = tk.Listbox(f_clie, height=4, font=self.ui_font)
    ...
    def crear_renglon_vta(self, mat):
        ...
        ent_cant = ttk.Entry(f, width=10, justify="center", font=self.ui_font)
        ...
        if mat:
            ...
            ent_nom = ttk.Entry(f, width=40, justify="center", font=self.ui_font)
            ent_nom.pack(side="left", padx=10)
            p_info = ent_nom

        ent_prec = ttk.Entry(f, width=15, justify="center", font=self.ui_font)
    ...
    def finalizar_venta(self, con_iva=False, orden_carga=False):
        ...
        for item in self.inputs_materiales:
            try:
                c_str = item["cant"].get()
                p_str = item["prec"].get()
                if not c_str or not p_str: continue
                c = float(c_str)
                p = float(p_str)
                if con_iva: p = p * 1.21
                if c > 0:
        ...
        if total <= 0: return messagebox.showwarning("Error", "Ingrese productos válidos")
        if orden_carga:
            with get_db_connection() as conn:
                c = conn.cursor()
                for p in detalle:
        ...
        with get_db_connection() as conn:
            ...
            c.execute("INSERT INTO ventas (cliente_nombre, fecha, total, detalle, con_iva) VALUES (?,?,?,?,?)", (self.cliente_actual_info, fecha, total, json.dumps(detalle), 1 if con_iva else 0))
            conn.commit()
        conn.close()
        self.actualizar_ui_stock()
        pdf_generado = None
        ...
        if messagebox.askyesno("Imprimir", "¿Desea imprimir el ticket de venta?"):
            ...
            for t_path in archivos_a_abrir:
                try:
                    if os.name == 'nt': os.startfile(t_path, "print")
                    elif os.name == 'posix': subprocess.run(['lpr', t_path], check=False)
                except Exception as e: print(f"Error al imprimir {t_path}: {e}")
    ...
    def generar_orden_carga(self, persona_info, fecha, productos, tabla_db):
        ...
        if loc: pdf.cell(0, 5, f"Ubicación: {loc}, {prov}", ln=True)
        if tel: pdf.cell(0, 5, f"Teléfono: {tel}", ln=True)
        pdf.ln(5)
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp: t_path = tmp.name
        pdf.output(t_path)
        return t_path
    ...
    def setup_compras(self):
        ...
        self.c_ent_busq = ttk.Entry(f_prov, font=self.ui_font)
        self.c_ent_busq.pack(fill="x", padx=5, pady=5)
        self.c_ent_busq.bind("<KeyRelease>", lambda e: self.actualizar_tablas())
        self.c_list_prov = tk.Listbox(f_prov, height=4, font=self.ui_font)
        ...
        def _on_wheel_c(event): self.c_canvas.yview_scroll(int(-1*(event.delta/120)), "units")
    ...
    def seleccionar_prov_compra(self, event):
        ...
        if sel: self.proveedor_actual_info = self.c_list_prov.get(sel)
    ...
    def finalizar_compra(self):
        ...
        for item in self.inputs_compras:
            try:
                ...
                if not n or c <= 0: continue
                sub = c * p
                total += sub
                detalle.append({"nombre": n, "cant": c, "prec": p, "sub": sub})
            except ValueError: continue
        if total <= 0: return messagebox.showwarning("Error", "Cargue productos válidos")
        fecha = datetime.now().strftime("%d-%m-%Y")
        with get_db_connection() as conn:
            c = conn.cursor()
        ...
        self.limpiar_formulario_compra(); self.actualizar_tablas()
    ...
    def generar_ticket_pdf(self, persona_info, fecha, total, productos, etiqueta, tabla_db, con_iva=False, preview=False):
        ...
        if preview:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp: pdf_path = tmp.name
            pdf.output(pdf_path); return pdf_path
    ...
    def setup_historiales(self):
        for tab, ent_attr, tree_attr, cmd_ver in [(self.tab_hven, "ent_busq_hven", "tree_hven", self.ver_ticket_hven), (self.tab_hcom, "ent_busq_hcom", "tree_hcom", self.ver_ticket_hcom)]:
            f_busq = ttk.Frame(tab); f_busq.pack(fill="x", padx=10, pady=5)
            ttk.Label(f_busq, text="Buscar:").pack(side="left")
            ent = ttk.Entry(f_busq, font=self.ui_font); ent.pack(side="left", fill="x", expand=True, padx=5); ent.bind("<KeyRelease>", lambda e: self.actualizar_tablas())
            ...
            tree.pack(fill="both", expand=True, padx=10, pady=10); tree.bind("<Double-1>", lambda e, cmd=cmd_ver: cmd())
    ...
    def ver_ticket_hven(self):
        ...
        with get_db_connection() as conn:
            c = conn.cursor(); c.execute("SELECT cliente_nombre, fecha, total, detalle, con_iva FROM ventas WHERE id=?", (v_id,)); r = c.fetchone()
        if r: self.abrir_archivo(self.generar_ticket_pdf(r[0], r[1], r[2], json.loads(r[3]), "Cliente", "clientes", bool(r[4]), preview=True))
    ...
    def ver_ticket_hcom(self):
        ...
        with get_db_connection() as conn:
            c = conn.cursor(); c.execute("SELECT proveedor_nombre, fecha, total, detalle FROM compras WHERE id=?", (c_id,)); r = c.fetchone()
        if r: self.abrir_archivo(self.generar_ticket_pdf(r[0], r[1], r[2], json.loads(r[3]), "Proveedor", "proveedores", preview=True))

    def setup_balance(self):
        f_bal = ttk.Frame(self.tab_bal); f_bal.pack(fill="both", expand=True, padx=10, pady=10)
        f_gen = ttk.LabelFrame(f_bal, text="Balances Generales"); f_gen.pack(fill="x", pady=5)
        anios = [str(a) for a in range(datetime.now().year, 2020, -1)]; meses = [f"{i:02d}" for i in range(1, 13)]
        for lbl, t in [("Ventas Periodo:", "ventas"), ("Compras Periodo:", "compras")]:
            f = ttk.Frame(f_gen); f.pack(fill="x", pady=2)
            ttk.Label(f, text=lbl, width=15).pack(side="left", padx=5)
            m = ttk.Combobox(f, values=meses, width=5); m.pack(side="left", padx=2)
            a = ttk.Combobox(f, values=anios, width=7); a.pack(side="left", padx=2); a.set(anios[0])
        ...
        f_ent = ttk.LabelFrame(f_bal, text="Balances por Entidad"); f_ent.pack(fill="x", pady=10)
        for lbl, lst_attr, ent_attr, anio_attr, cmd_todo, cmd_anio in [("Cliente:", "bal_list_clie", "bal_ent_clie", "bal_anio_clie", self.gen_bal_cliente, self.gen_bal_cliente_anio), ("Proveedor:", "bal_list_prov", "bal_ent_prov", "bal_anio_prov", self.gen_bal_proveedor, self.gen_bal_proveedor_anio)]:
            f = ttk.Frame(f_ent); f.pack(fill="x", pady=5, padx=10)
            ttk.Label(f, text=f"Buscar {lbl}", width=18).pack(side="left")
            e = ttk.Entry(f, width=25, font=self.ui_font); e.pack(side="left", padx=5); e.bind("<KeyRelease>", lambda ev: self.actualizar_tablas()); setattr(self, ent_attr, e)
            l = tk.Listbox(f, height=6, font=self.ui_font); l.pack(side="left", fill="x", expand=True, padx=5); setattr(self, lst_attr, l)
            fb = ttk.Frame(f); fb.pack(side="left", padx=5)
            ttk.Button(fb, text="Todo", width=10, command=cmd_todo).pack(pady=2)
            fa = ttk.Frame(fb); fa.pack(pady=2)
            c = ttk.Combobox(fa, values=anios, width=6, state="readonly"); c.pack(side="left"); c.set(anios[0]); setattr(self, anio_attr, c)
            ttk.Button(fa, text="Año", width=5, command=cmd_anio).pack(side="left", padx=2)

    def gen_bal_cliente(self): self._gen_bal_e("ventas", "cliente_nombre", self.bal_list_clie, "Cliente")
    def gen_bal_cliente_anio(self): self._gen_bal_e("ventas", "cliente_nombre", self.bal_list_clie, "Cliente", self.bal_anio_clie.get())
    def gen_bal_proveedor(self): self._gen_bal_e("compras", "proveedor_nombre", self.bal_list_prov, "Proveedor")
    def gen_bal_proveedor_anio(self): self._gen_bal_e("compras", "proveedor_nombre", self.bal_list_prov, "Proveedor", self.bal_anio_prov.get())

    def _gen_bal_e(self, tabla, col, lst, tag, anio=None):
        sel = lst.curselection()
        if not sel: return messagebox.showwarning("Aviso", "Seleccione una entidad")
        info = lst.get(sel)
        with get_db_connection() as conn:
            c = conn.cursor()
            if anio: c.execute(f"SELECT total, detalle FROM {tabla} WHERE {col} = ? AND fecha LIKE ?", (info, f"%{anio}"))
            else: c.execute(f"SELECT total, detalle FROM {tabla} WHERE {col} = ?", (info,))
            res = c.fetchall()
        if not res: return messagebox.showinfo("Aviso", "Sin datos")
        tot, acc = 0, {}
        for r in res:
            tot += r[0]
            for p in json.loads(r[1]):
                n = p['nombre'].upper(); acc[n] = acc.get(n, {'cant': 0, 'sub': 0}); acc[n]['cant'] += p['cant']; acc[n]['sub'] += p['sub']
        pdf = FPDF(); pdf.add_page(); pdf.set_font("Arial", 'B', 14); pdf.cell(0, 10, f"BALANCE - {tag.upper()}", ln=True, align='C')
        pdf.set_font("Arial", size=10); pdf.cell(0, 5, f"Entidad: {info}", ln=True);
        if anio: pdf.cell(0, 5, f"Año: {anio}", ln=True)
        pdf.ln(5); pdf.set_font("Arial", 'B', 10)
        pdf.cell(100, 7, "Producto", border=1); pdf.cell(20, 7, "Cant", border=1, align='C'); pdf.cell(30, 7, "P.Prom ($)", border=1, align='C'); pdf.cell(40, 7, "Subtotal ($)", border=1, align='C', ln=True)
        pdf.set_font("Arial", size=10)
        for k, v in acc.items():
            p_p = v['sub'] / v['cant'] if v['cant'] > 0 else 0
            x, y = pdf.get_x(), pdf.get_y(); pdf.multi_cell(100, 6, k, border=1)
            h = pdf.get_y() - y; pdf.set_xy(x + 100, y)
            pdf.cell(20, h, f"{v['cant']:.0f}", border=1, align='C'); pdf.cell(30, h, self.formato_moneda(p_p), border=1, align='C'); pdf.cell(40, h, self.formato_moneda(v['sub']), border=1, align='C', ln=True)
        pdf.ln(5); pdf.set_font("Arial", 'B', 10); pdf.cell(150, 7, "TOTAL ($):", align='R'); pdf.cell(40, 7, self.formato_moneda(tot), border=1, align='C', ln=True)
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp: t_path = tmp.name
        pdf.output(t_path); self.abrir_archivo(t_path)

    def gen_bal(self, tabla, filtro):
        with get_db_connection() as conn:
            c = conn.cursor(); c.execute(f"SELECT total, detalle FROM {tabla} WHERE fecha LIKE ?", (f"%{filtro}%",)); res = c.fetchall()
        if not res: return messagebox.showinfo("Aviso", "Sin datos")
        tot, acc = 0, {}
        for r in res:
            tot += r[0]
            for p in json.loads(r[1]):
                n = p['nombre'].upper(); acc[n] = acc.get(n, {'cant': 0, 'sub': 0}); acc[n]['cant'] += p['cant']; acc[n]['sub'] += p['sub']
        pdf = FPDF(); pdf.add_page(); pdf.set_font("Arial", 'B', 14); pdf.cell(0, 10, f"BALANCE {tabla.upper()}", ln=True, align='C')
        pdf.set_font("Arial", size=10); pdf.cell(0, 5, f"Periodo: {filtro}", ln=True); pdf.ln(5); pdf.set_font("Arial", 'B', 10)
        pdf.cell(100, 7, "Producto", border=1); pdf.cell(20, 7, "Cant", border=1, align='C'); pdf.cell(30, 7, "P.Prom ($)", border=1, align='C'); pdf.cell(40, 7, "Subtotal ($)", border=1, align='C', ln=True)
        pdf.set_font("Arial", size=10)
        for k, v in acc.items():
            p_p = v['sub'] / v['cant'] if v['cant'] > 0 else 0
    ...
    def setup_cuenta_corriente(self):
        f_clie = ttk.LabelFrame(self.tab_cc, text="1. Seleccionar Cliente"); f_clie.pack(fill="x", padx=10, pady=5)
        self.cc_ent_busq = ttk.Entry(f_clie, font=self.ui_font); self.cc_ent_busq.pack(fill="x", padx=5, pady=5); self.cc_ent_busq.bind("<KeyRelease>", lambda e: self.actualizar_tablas())
        self.cc_list_clie = tk.Listbox(f_clie, height=4, font=self.ui_font); self.cc_list_clie.pack(fill="x", padx=5, pady=2); self.cc_list_clie.bind("<<ListboxSelect>>", self.seleccionar_cliente_cc)
        f_movs = ttk.LabelFrame(self.tab_cc, text="2. Historial de Movimientos"); f_movs.pack(fill="both", expand=True, padx=10, pady=5)
        ...
        for c in cols: self.tree_cc.heading(c, text=c); self.tree_cc.column(c, width=100, anchor="center")
        self.tree_cc.pack(fill="both", expand=True, padx=5, pady=5); self.tree_cc.bind("<Double-1>", self.ver_ticket_desde_cc)
        f_pago = ttk.LabelFrame(self.tab_cc, text="3. Registrar Pago"); f_pago.pack(fill="x", padx=10, pady=5)
        ttk.Label(f_pago, text="Fecha:").grid(row=0, column=0, padx=5, pady=5)
        self.ent_cc_fecha = ttk.Entry(f_pago, font=self.ui_font); self.ent_cc_fecha.grid(row=0, column=1, padx=5, pady=5); self.ent_cc_fecha.insert(0, datetime.now().strftime("%d-%m-%Y"))
        ttk.Label(f_pago, text="Monto ($):").grid(row=0, column=2, padx=5, pady=5)
        self.ent_cc_monto = ttk.Entry(f_pago, font=self.ui_font); self.ent_cc_monto.grid(row=0, column=3, padx=5, pady=5)
        ttk.Label(f_pago, text="Obs:").grid(row=0, column=4, padx=5, pady=5)
        self.ent_cc_obs = ttk.Entry(f_pago, font=self.ui_font); self.ent_cc_obs.grid(row=0, column=5, padx=5, pady=5)
    ...
    def seleccionar_cliente_cc(self, event):
        ...
        if sel: self.cliente_cc_info = self.cc_list_clie.get(sel); self.cargar_historial_cc()

    def registrar_pago(self):
        if not hasattr(self, 'cliente_cc_info'): return messagebox.showwarning("Error", "Seleccione un cliente")
        try: monto = float(self.ent_cc_monto.get().strip())
        except ValueError: return messagebox.showwarning("Error", "Monto inválido")
        with get_db_connection() as conn:
            conn.cursor().execute("INSERT INTO pagos (cliente_nombre, fecha, monto, observaciones) VALUES (?,?,?,?)", (self.cliente_cc_info, self.ent_cc_fecha.get().strip(), monto, self.ent_cc_obs.get().strip())); conn.commit()
        messagebox.showinfo("Éxito", "Pago registrado"); self.ent_cc_monto.delete(0, tk.END); self.ent_cc_obs.delete(0, tk.END); self.cargar_historial_cc(); self.actualizar_tablas()

    def eliminar_pago(self):
        sel = self.tree_cc.selection()
        if not sel or self.tree_cc.item(sel)['values'][2] != "Pago": return messagebox.showwarning("Error", "Seleccione un Pago")
        if messagebox.askyesno("Confirmar", "¿Eliminar pago?"):
            with get_db_connection() as conn: conn.cursor().execute("DELETE FROM pagos WHERE id=?", (self.tree_cc.item(sel)['values'][0],)); conn.commit()
            self.cargar_historial_cc(); self.actualizar_tablas()

    def ver_ticket_desde_cc(self, event):
        sel = self.tree_cc.selection()
        if not sel or self.tree_cc.item(sel)['values'][2] != "Venta": return
        with get_db_connection() as conn: r = conn.cursor().execute("SELECT cliente_nombre, fecha, total, detalle, con_iva FROM ventas WHERE id=?", (self.tree_cc.item(sel)['values'][0],)).fetchone()
        if r: self.abrir_archivo(self.generar_ticket_pdf(r[0], r[1], r[2], json.loads(r[3]), "Cliente", "clientes", bool(r[4]), preview=True))

    def cargar_historial_cc(self):
        for i in self.tree_cc.get_children(): self.tree_cc.delete(i)
        if not hasattr(self, 'cliente_cc_info'): return
        movs = []
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute("SELECT id, fecha, total FROM ventas WHERE cliente_nombre = ?", (self.cliente_cc_info,))
            for vid, f, t in c.fetchall(): movs.append({'id': vid, 'fecha': f, 'tipo': 'Venta', 'detalle': 'Venta ticket', 'debe': t, 'haber': 0})
            c.execute("SELECT id, fecha, monto, observaciones FROM pagos WHERE cliente_nombre = ?", (self.cliente_cc_info,))
            for pid, f, m, o in c.fetchall(): movs.append({'id': pid, 'fecha': f, 'tipo': 'Pago', 'detalle': o, 'debe': 0, 'haber': m})
        try: movs.sort(key=lambda x: datetime.strptime(x['fecha'], "%d-%m-%Y"))
        except: pass
        saldo = 0
        for m in movs:
    ...
    def setup_deudores(self):
        f = ttk.LabelFrame(self.tab_deud, text="Deudores"); f.pack(fill="both", expand=True, padx=10, pady=10)
        cols = ("Nombre", "CUIT/DNI", "Teléfono", "Provincia", "Localidad", "Saldo")
        self.tree_deudores = ttk.Treeview(f, columns=cols, show='headings')
        for c in cols: self.tree_deudores.heading(c, text=c); self.tree_deudores.column(c, width=120, anchor="center")
        self.tree_deudores.pack(fill="both", expand=True, padx=5, pady=5)
        bf = ttk.Frame(self.tab_deud); bf.pack(pady=10)
    ...
    def actualizar_deudores(self):
        ...
        with get_db_connection() as conn:
            c = conn.cursor(); c.execute("SELECT nombre, apellido, cuit_dni, telefono, provincia, localidad FROM clientes")
            for nom, ape, cuit, tel, prov, loc in c.fetchall():
                ...
                if v - p > 0.01: self.tree_deudores.insert("", "end", values=(f"{nom} {ape}", cuit, tel, prov, loc, self.formato_moneda(v - p)))
            ...
            if v_cf - p_cf > 0.01: self.tree_deudores.insert("", "end", values=("Consumidor Final", "-", "-", "-", "-", self.formato_moneda(v_cf - p_cf)))

    def generar_pdf_deudores(self):
        items = [self.tree_deudores.item(i)['values'] for i in self.tree_deudores.get_children()]
        if not items: return messagebox.showinfo("Aviso", "No hay deudores")
        pdf = FPDF(orientation='L'); pdf.add_page(); pdf.set_font("Arial", 'B', 16); pdf.cell(0, 10, "LISTADO DE DEUDORES", ln=True, align='C')
        pdf.set_font("Arial", '', 10); pdf.cell(0, 10, f"Fecha: {datetime.now().strftime('%d-%m-%Y %H:%M')}", ln=True, align='R'); pdf.ln(5)
        pdf.set_font("Arial", 'B', 10); cols = ["Nombre", "CUIT/DNI", "Teléfono", "Provincia", "Localidad", "Saldo"]; ws = [60, 40, 40, 40, 40, 40]
        for i, col in enumerate(cols): pdf.cell(ws[i], 10, col, border=1, align='C')
        pdf.ln(); pdf.set_font("Arial", '', 10); td = 0
        for item in items:
            for i, val in enumerate(item): pdf.cell(ws[i], 10, str(val).encode('latin-1', 'replace').decode('latin-1'), border=1, align='C')
            pdf.ln(); td += float(str(item[5]).replace(".", "").replace(",", "."))
        pdf.ln(5); pdf.set_font("Arial", 'B', 12); pdf.cell(0, 10, f"DEUDA TOTAL: $ {self.formato_moneda(td)}", ln=True, align='R')
        path = os.path.join(os.getcwd(), "pdf", f"Deudores_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"); pdf.output(path); self.abrir_archivo(path)

    def setup_presupuestos(self):
        pw = ttk.PanedWindow(self.tab_pres, orient="vertical"); pw.pack(fill="both", expand=True)
        f_n = ttk.Frame(pw); f_h = ttk.Frame(pw); pw.add(f_n, weight=3); pw.add(f_h, weight=2)
        f_t = ttk.Frame(f_n); f_t.pack(fill="x", padx=10, pady=5)
        f_c = ttk.LabelFrame(f_t, text="1. Cliente"); f_c.pack(side="left", fill="both", expand=True, padx=5)
        self.p_ent_busq = ttk.Entry(f_c, font=self.ui_font); self.p_ent_busq.pack(fill="x", padx=5, pady=5); self.p_ent_busq.bind("<KeyRelease>", lambda e: self.actualizar_tablas())
        self.p_list_clie = tk.Listbox(f_c, height=4, font=self.ui_font); self.p_list_clie.pack(fill="x", padx=5, pady=2); self.p_list_clie.bind("<<ListboxSelect>>", self.seleccionar_cliente_presupuesto)
        f_ex = ttk.LabelFrame(f_t, text="2. Datos Manuales"); f_ex.pack(side="left", fill="both", expand=True, padx=5)
        self.p_txt_clie = tk.Text(f_ex, height=4, width=30, font=self.ui_font); self.p_txt_clie.pack(fill="both", expand=True, padx=5, pady=5)
        f_f = ttk.LabelFrame(f_t, text="3. Fecha"); f_f.pack(side="left", fill="y", padx=5)
        self.p_ent_fecha = ttk.Entry(f_f, width=12, font=self.ui_font); self.p_ent_fecha.pack(padx=10, pady=10); self.p_ent_fecha.insert(0, datetime.now().strftime("%d-%m-%Y")); self.p_ent_fecha.bind("<KeyRelease>", self.validar_fecha_presupuesto)
        f_b = ttk.Frame(f_n); f_b.pack(fill="both", expand=True, padx=10, pady=5)
        f_bt = ttk.Frame(f_b); f_bt.pack(side="right", fill="y", padx=(15, 5), pady=5)
        f_m = ttk.LabelFrame(f_b, text="4. Productos"); f_m.pack(side="left", fill="both", expand=True, padx=(0, 5))
        f_hd = ttk.Frame(f_m); f_hd.pack(fill="x")
        for t, w in [("Cantidad", 10), ("Descripción", 40), ("Precio ($)", 16)]: ttk.Label(f_hd, text=t, width=w, anchor="center").pack(side="left", padx=10)
        self.p_canvas = tk.Canvas(f_m); self.p_scrollbar = ttk.Scrollbar(f_m, command=self.p_canvas.yview); self.p_scroll_f = ttk.Frame(self.p_canvas)
        self.p_canvas.create_window((0, 0), window=self.p_scroll_f, anchor="nw"); self.p_canvas.configure(yscrollcommand=self.p_scrollbar.set); self.p_canvas.pack(side="left", fill="both", expand=True); self.p_scrollbar.pack(side="right", fill="y")
        ...
        for mat in self.lista_materiales + [None]*5:
            f = ttk.Frame(self.p_scroll_f); f.pack(fill="x", pady=1)
            c = ttk.Entry(f, width=10, justify="center", font=self.ui_font); c.pack(side="left", padx=10); c.bind("<KeyRelease>", lambda e: self.actualizar_total_presupuesto())
            if mat: ttk.Label(f, text=mat, width=40).pack(side="left", padx=10); p_info = mat
            else: p_info = ttk.Entry(f, width=40, justify="center", font=self.ui_font); p_info.pack(side="left", padx=10)
            p = ttk.Entry(f, width=15, justify="center", font=self.ui_font); p.pack(side="left", padx=10); p.bind("<KeyRelease>", lambda e: self.actualizar_total_presupuesto())
            self.inputs_presupuesto.append({"prod": p_info, "cant": c, "prec": p, "es_extra": mat is None})
        f_iva = ttk.Frame(f_bt); f_iva.pack(fill="x", pady=5)
        self.p_var_iva = tk.BooleanVar(value=True); ttk.Checkbutton(f_iva, text="IVA", variable=self.p_var_iva, command=self.actualizar_total_presupuesto).pack(side="left", padx=2)
        self.p_ent_iva_porc = ttk.Entry(f_iva, width=5, font=self.ui_font); self.p_ent_iva_porc.pack(side="left", padx=2); self.p_ent_iva_porc.insert(0, "21"); self.p_ent_iva_porc.bind("<KeyRelease>", lambda e: self.actualizar_total_presupuesto())
        self.lbl_p_total = ttk.Label(f_bt, text="TOTAL: $ 0,00", font=("Arial Black", 14, "bold"), foreground="green"); self.lbl_p_total.pack(pady=8)
        ...
        f_hist = ttk.LabelFrame(f_h, text="Historial"); f_hist.pack(fill="both", expand=True, padx=10, pady=5)
        f_bh = ttk.Frame(f_hist); f_bh.pack(fill="x", padx=5, pady=2); self.p_ent_busq_h = ttk.Entry(f_bh, font=self.ui_font); self.p_ent_busq_h.pack(side="left", fill="x", expand=True); self.p_ent_busq_h.bind("<KeyRelease>", lambda e: self.actualizar_tablas())
        bfh = ttk.Frame(f_hist); bfh.pack(pady=5); ttk.Button(bfh, text="VER PDF", command=self.reimprimir_presupuesto).pack(side="left", padx=5); ttk.Button(bfh, text="ELIMINAR", command=self.eliminar_presupuesto).pack(side="left", padx=5)
        self.tree_hpres = ttk.Treeview(f_hist, columns=("ID", "Cliente", "Fecha", "Total"), show='headings')
        for c in ("ID", "Cliente", "Fecha", "Total"): self.tree_hpres.heading(c, text=c); self.tree_hpres.column(c, width=150, anchor="center")
        self.tree_hpres.pack(fill="both", expand=True, padx=5, pady=5); self.tree_hpres.bind("<Double-1>", lambda e: self.reimprimir_presupuesto())

    def validar_fecha_presupuesto(self, event):
        s = ''.join(c for c in self.p_ent_fecha.get().replace("-", "") if c.isdigit())[:8]
        if len(s) >= 4: s = f"{s[:2]}-{s[2:4]}-{s[4:]}"
        elif len(s) >= 2: s = f"{s[:2]}-{s[2:]}"
        self.p_ent_fecha.delete(0, tk.END); self.p_ent_fecha.insert(0, s)

    def actualizar_total_presupuesto(self):
        t, i_en = 0, self.p_var_iva.get()
        try: i_p = float(self.p_ent_iva_porc.get()) / 100
        except: i_p = 0.21
        for it in self.inputs_presupuesto:
            try:
                c, p = float(it["cant"].get()), float(it["prec"].get())
                if c > 0 and p > 0: sub = c * p; t += sub + (sub * i_p if i_en else 0)
            except ValueError: continue
    ...
    def seleccionar_cliente_presupuesto(self, event):
        ...
        if sel: self.cliente_presupuesto_info = self.p_list_clie.get(sel)

    def finalizar_presupuesto(self):
        t_s, det, i_en = 0, [], self.p_var_iva.get()
        try: i_p = float(self.p_ent_iva_porc.get())
        except: i_p = 21.0
        for it in self.inputs_presupuesto:
            try:
                c, p = float(it["cant"].get()), float(it["prec"].get())
                n = it["prod"] if not it["es_extra"] else it["prod"].get().strip()
                if c > 0 and n: sub = c * p; t_s += sub; det.append({"nombre": n, "cant": c, "prec": p, "sub": sub})
            except ValueError: continue
        if t_s <= 0: return messagebox.showwarning("Error", "Ingrese productos")
        t_f = t_s + (t_s * i_p / 100 if i_en else 0)
        cli = f"{getattr(self, 'cliente_presupuesto_info', '')}\n{self.p_txt_clie.get('1.0', tk.END).strip()}".strip()
        if not cli: return messagebox.showwarning("Error", "Ingrese cliente")
        with get_db_connection() as conn: conn.cursor().execute("INSERT INTO presupuestos (cliente_info, fecha, total, detalle) VALUES (?,?,?,?)", (cli, self.p_ent_fecha.get().strip(), t_f, json.dumps({"detalle": det, "iva_habilitado": i_en, "porc_iva": i_p, "total_sin_iva": t_s}))); conn.commit()
        self.abrir_archivo(self.generar_presupuesto_pdf(cli, self.p_ent_fecha.get().strip(), t_f, det, i_en, i_p, t_s))
        self.limpiar_formulario_presupuesto(); self.actualizar_tablas()

    def limpiar_formulario_presupuesto(self):
        for w in [self.p_ent_busq, self.p_ent_fecha]: w.delete(0, tk.END)
        self.p_list_clie.delete(0, tk.END); self.p_txt_clie.delete("1.0", tk.END); self.p_ent_fecha.insert(0, datetime.now().strftime("%d-%m-%Y")); self.p_var_iva.set(True); self.p_ent_iva_porc.delete(0, tk.END); self.p_ent_iva_porc.insert(0, "21"); self.lbl_p_total.config(text="TOTAL: $ 0,00")
        if hasattr(self, 'cliente_presupuesto_info'): del self.cliente_presupuesto_info
        for it in self.inputs_presupuesto:
            it["cant"].delete(0, tk.END); it["prec"].delete(0, tk.END)
            if it["es_extra"]: it["prod"].delete(0, tk.END)

    def reimprimir_presupuesto(self):
        sel = self.tree_hpres.selection()
        if not sel: return
        with get_db_connection() as conn: r = conn.cursor().execute("SELECT cliente_info, fecha, total, detalle FROM presupuestos WHERE id=?", (self.tree_hpres.item(sel)['values'][0],)).fetchone()
        if r:
            try:
                d = json.loads(r[3])
                if isinstance(d, dict) and "detalle" in d: path = self.generar_presupuesto_pdf(r[0], r[1], r[2], d["detalle"], d.get("iva_habilitado", False), d.get("porc_iva", 21), d.get("total_sin_iva", r[2]))
                else: path = self.generar_presupuesto_pdf(r[0], r[1], r[2], d)
    ...
    def eliminar_presupuesto(self):
        ...
        if sel and messagebox.askyesno("Confirmar", "¿Eliminar?"):
            with get_db_connection() as conn: conn.cursor().execute("DELETE FROM presupuestos WHERE id=?", (self.tree_hpres.item(sel)['values'][0],)); conn.commit()
            self.actualizar_tablas()

    def generar_presupuesto_pdf(self, cli, fecha, total, prods, iva_en=False, iva_p=21.0, t_s=0):
        os.makedirs("presupuestos", exist_ok=True); pdf = FPDF(); pdf.add_page()
        logo = next((p for p in ["logo.png", "logo.jpg", "logo apicolavallejos con cuit.png"] if os.path.exists(p)), None)
        if logo:
            try: pdf.image(logo, 10, 8, 33)
            except: pass
        pdf.set_font("Arial", 'B', 15); pdf.cell(0, 10, 'PRESUPUESTO', ln=True, align='C'); pdf.ln(5)
        pdf.set_font("Arial", '', 10); pdf.cell(0, 5, f"Fecha: {fecha}", ln=True, align='R')
        pdf.set_font("Arial", 'B', 12); pdf.cell(0, 7, "Datos del Cliente:", ln=True)
        pdf.set_font("Arial", '', 10)
        for line in cli.split('\n'): pdf.cell(0, 5, line.encode('latin-1', 'replace').decode('latin-1'), ln=True)
        pdf.ln(5); pdf.set_font("Arial", 'B', 10); pdf.cell(100, 7, "Producto", border=1); pdf.cell(20, 7, "Cant", border=1, align='C'); pdf.cell(30, 7, "P.Unit ($)", border=1, align='C'); pdf.cell(40, 7, "Subtotal ($)", border=1, align='C', ln=True)
        pdf.set_font("Arial", size=10)
        for p in prods:
            x, y = pdf.get_x(), pdf.get_y(); pdf.multi_cell(100, 6, p['nombre'], border=1)
            h = pdf.get_y() - y; pdf.set_xy(x + 100, y)
            pdf.cell(20, h, f"{p['cant']:.0f}", border=1, align='C'); pdf.cell(30, h, self.formato_moneda(p['prec']), border=1, align='C'); pdf.cell(40, h, self.formato_moneda(p['sub']), border=1, align='C', ln=True)
        pdf.ln(5); pdf.set_font("Arial", 'B', 10)
        if iva_en:
            pdf.cell(150, 6, "SUBTOTAL SIN IVA ($):", align='R'); pdf.cell(40, 6, self.formato_moneda(t_s), border=1, align='C', ln=True)
            pdf.cell(150, 6, f"IVA ({iva_p}%) ($):", align='R'); pdf.cell(40, 6, self.formato_moneda(total - t_s), border=1, align='C', ln=True)
            pdf.cell(150, 6, "TOTAL CON IVA ($):", align='R')
        else: pdf.cell(150, 6, "TOTAL ($):", align='R')
        pdf.cell(40, 6, self.formato_moneda(total), border=1, align='C', ln=True)
        path = os.path.join("presupuestos", f"Pres_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"); pdf.output(path); return path
    ...
    def actualizar_tablas(self):
        with get_db_connection() as conn:
            ...
            for tree, ent_b, tabla, col, ref_table in [(self.tree_hven, self.ent_busq_hven, "ventas", "cliente_nombre", "clientes"), (self.tree_hcom, self.ent_busq_hcom, "compras", "proveedor_nombre", "proveedores")]:
                ...
                for r in c.fetchall():
                    pts = r[1].split(" ("); nom = pts[0].split(" - CUIT/DNI: ")[0]; ref = c.execute(f"SELECT nombre, apellido, cuit_dni FROM {ref_table} WHERE nombre || ' ' || apellido = ?", (nom,)).fetchone()
                    if ref: n, a, cd = ref
                    else: p = nom.split(" ", 1); n, a, cd = p[0], (p[1] if len(p)>1 else ""), ""
                    ubi = pts[1].replace(")", "") if len(pts)>1 else ""; prov, loc = ubi.split(" - ") if " - " in ubi else (ubi, "")
            ...
            for lst, ent, tbl in [(self.v_list_clie, self.v_ent_busq, "clientes"), (self.c_list_prov, self.c_ent_busq, "proveedores"), (self.bal_list_clie, self.bal_ent_clie, "clientes"), (self.bal_list_prov, self.bal_ent_prov, "proveedores"), (self.cc_list_clie, self.cc_ent_busq, "clientes"), (self.p_list_clie, self.p_ent_busq, "clientes")]:
                lst.delete(0, tk.END); b = f"%{ent.get()}%"
                if tbl == "clientes" and ent.get().lower() in "consumidor final" and c.execute("SELECT 1 FROM ventas WHERE cliente_nombre = 'Consumidor Final'").fetchone(): lst.insert(tk.END, "Consumidor Final")
                elif tbl == "proveedores" and ent.get().lower() in "proveedor genérico" and c.execute("SELECT 1 FROM compras WHERE proveedor_nombre = 'Proveedor Genérico'").fetchone(): lst.insert(tk.END, "Proveedor Genérico")
                ...
                for n, a, cdni, p, l in c.fetchall(): lst.insert(tk.END, f"{n} {a}" + (f" - CUIT/DNI: {cdni}" if cdni else "") + f" ({p} - {l})")
            self.actualizar_deudores()
            if hasattr(self, 'cliente_cc_info'): self.cargar_historial_cc()
            for i in self.tree_hpres.get_children(): self.tree_hpres.delete(i)
            ...
            for r in c.fetchall(): self.tree_hpres.insert("", "end", values=(r[0], r[1].split('\n')[0], r[2], self.formato_moneda(r[3])))

    def limpiar_formulario_venta(self):
        for w in [self.v_ent_busq, self.v_list_clie]: w.delete(0, tk.END) if hasattr(w, 'delete') else None
        self.cliente_actual_info = "Consumidor Final"
        for i in self.inputs_materiales: i["cant"].delete(0, tk.END); i["prec"].delete(0, tk.END); i["prod"].delete(0, tk.END) if i["es_extra"] else None

    def limpiar_formulario_compra(self):
        self.c_ent_busq.delete(0, tk.END); self.c_list_prov.delete(0, tk.END); self.proveedor_actual_info = "Proveedor Genérico"
        for i in self.inputs_compras: i["cant"].delete(0, tk.END); i["prec"].delete(0, tk.END); i["prod"].delete(0, tk.END)

if __name__ == "__main__": root = tk.Tk(); app = AppApicola(root); root.mainloop()
